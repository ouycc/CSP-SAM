from segment_anything_cnn_refine_decoder_2 import sam_model_registry
import os
...
import argparse
import os
from utils import FocalDiceloss_IoULoss, generate_point, save_masks,sam_refiner
...
def parse_args():
    ...
    parser.add_argument("--run_name", type=str, default="Dynamic_log", help="run model name")
    parser.add_argument("--batch_size", type=int, default=1, help="batch size")
    parser.add_argument("--image_size", type=int, default=256, help="image_size")
    parser.add_argument('--device', type=str, default='cuda')
    parser.add_argument("--data_path", type=str, default="/media/Storage4/yc/SAM-Med2D/Dynamic", help="train data path") 
    parser.add_argument("--metrics", nargs='+', default=['iou', 'dice'], help="metrics")
    parser.add_argument("--model_type", type=str, default="vit_b", help="sam model_type")
    parser.add_argument("--sam_checkpoint", type=str, default="/media/Storage4/yc/SAM-Med2D/workdir/models/sam-med2d-Dynamic_cnn_refine_decoder/epoch468_sam.pth", help="sam checkpoint")
...
def main(args):
    ...
    for i, batched_input in enumerate(test_pbar):
        ...
        # coarse_masks = (torch.sigmoid(masks) > 0.75).to(torch.uint8)
        ...
        # low_res = []
        # for i, idx in enumerate(max_indexs):
        #     low_res.append(sam_masks3[i:i+1, idx])
        # low_res_masks = torch.stack(low_res, 0)
        # masks, pad = postprocess_masks(low_res_masks, args.image_size, original_size)
