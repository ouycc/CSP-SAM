...
class DeformableCrossAttentionMMCV(nn.Module):
    def __init__(self, embed_dim=768, num_heads=8, num_points=4):
        super().__init__()
    ...
    def forward(self, query_feats, reference_feats):
        """
        query_feats: (B, H, W, C)
        reference_feats: (B, H, W, C)
        """
        B, H, W, C = query_feats.shape
        spatial_shape = (H, W)

        query = self.query_proj(query_feats).reshape(B, H * W, C)
        value = self.value_proj(reference_feats).reshape(B, H * W, C)

        spatial_shapes = value.new_tensor([list(spatial_shape)], dtype=torch.long)
        level_start_index = value.new_tensor([0], dtype=torch.long)

        grid_y, grid_x = torch.meshgrid(
            torch.linspace(0.5, H - 0.5, H) / H,
            torch.linspace(0.5, W - 0.5, W) / W,
            indexing="ij"
        )
        reference_points = torch.stack((grid_x, grid_y), -1).reshape(1, H * W, 1, 2).to(query.device)
...
class BilateralDeformableAttention(nn.Module):
    def __init__(self, dim=768, num_heads=8, num_points=4):
        super().__init__()
        self.norm_q = nn.LayerNorm(dim)
        self.norm_kv = nn.LayerNorm(dim)

        self.attn_vit = DeformableCrossAttentionMMCV(dim, num_heads, num_points)
        self.attn_cnn = DeformableCrossAttentionMMCV(dim, num_heads, num_points)
    ...
    def forward(self, Fd, Fs):
        ...
        Fcd = self.attn_vit(Fd_, Fs_)
        Fsc = self.attn_cnn(Fs_, Fd_)

        # residual + FFN
        Fd_out = Fd + Fcd
        Fs_out = Fs + Fsc

        Fd_out = Fd_out + self.ffn_vit(Fd_out)
        Fs_out = Fs_out + self.ffn_cnn(Fs_out)

        return Fd_out, Fs_out
...
class ParaBlock(nn.Module):
    ...
    def __init__(
        ...
        # ------------------ New Adapters (only used when window_size == 0) ------------------
        if self.window_size == 0:
            self.MLP_Adapter = Adapter(dim, skip_connect=False)
            self.Space_Adapter = qkvAttention(dim=dim, num_heads=num_heads)
            self.refine_Adapter = SingleConv(in_channels=dim, out_channels=dim)
            self.cross_attn = BilateralDeformableAttention(dim=dim, num_heads=num_heads, num_points=4)
            self.scale = 0.5
        # -------------------------------------------------------------------------------------

    def forward(self, x: torch.Tensor, cnnx: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        x: ViT features (B, H, W, C)
        cnnx: CNN features (B, H, W, C)
        """
        shortcut = x
        x = self.norm1(x)
        ...
        if self.window_size == 0:
            ...
            # --- Positional Fusion with Spatial Adapter ---
